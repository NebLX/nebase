...
static int set_pid(int fd)
{
	...
	if (ftruncate(fd, 0) == -1) {
		neb_syslogl(LOG_ERR, "ftruncate: %m");
		return -1;
	}
...
int neb_pidfile_open(const char *path, pid_t *locker)
{
	*locker = 0;
	...
	if (fd == -1) {
		neb_syslogl(LOG_ERR, "open(%s): %m", path);
		return -1;
	}
	struct flock lock = {
		.l_type = F_WRLCK,
	...
	if (fcntl(fd, F_GETLK, &lock) == -1) {
		neb_syslogl(LOG_ERR, "fcntl(F_GETLK): %m");
		close(fd);
		return -1;
	}
...
pid_t neb_pidfile_write(int fd)
{
	...
	if (fcntl(fd, F_SETLK, &lock) == -1) {
		...
		case EAGAIN:
			break;
		default:
			neb_syslogl(LOG_ERR, "fcntl(F_SETLK): %m");
			return -1;
			break;
		}
		return get_pid(fd);
